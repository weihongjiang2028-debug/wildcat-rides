(() => {
  ...
  const sb =
    window.__nu_carpool_sb ||
    (window.__nu_carpool_sb = window.supabase.createClient(
      SUPABASE_URL,
      SUPABASE_ANON_KEY
    ));
  ...
  function windowMidMinutes(from_t, to_t) {
    return Math.round((timeToMinutes(from_t) + timeToMinutes(to_t)) / 2);
  }
  ...
  function makeMessageTemplate(ride) {
    const dateStr = isoToMDY(ride.ride_date);
    const windowStr = `${formatTimeAMPM(ride.from_time)}–${formatTimeAMPM(ride.to_time)}`;
    return `Hi! I just joined your carpool on ${dateStr} (${ride.direction}, ${ride.airport}). My leave window is ${windowStr}. Where should we meet, and what time should we confirm? Thanks!`;
  }

  function fillJoinDoneStep1(ride) {
    const poster = riderLabelFromEmail(ride.owner_email).replace("👤 ", "");
    const msg = makeMessageTemplate(ride);
    ...
    __refreshInFlight = withTimeout(
      (async () => {
        ...
        if (force) {
          try {
            const timeout = (ms) =>
              new Promise((_, rej) => setTimeout(() => rej(new Error("getUser timeout")), ms));
  ...
  async function maybeResumePendingAction() {
    ...
    if (pending.type === "POST") {
      ...
      await doCancelJoin(pending.payload.rideId, null);
    } else if (pending.type === "LOAD_MY") {
      activateTab("my");
      await loadMyRides();
    } else if (pending.type === "CANCEL_RIDE") {
      await doCancelRideAsHost(pending.payload.rideId, null);
    }
  }
  ...
  async function searchRides() {
    ...
    const enriched = list.map((r) => {
      ...
      return { ...r, seats_filled: filled, dist, alreadyJoined };
    });

    enriched.sort((a, b) => {
      if (tMin !== null) return (a.dist ?? 0) - (b.dist ?? 0);
  ...
  function renderResults(rides) {
    ...
    for (const r of rides) {
      ...
      const joinBtnHtml = joined
        ? `<button data-cancel="${r.id}" class="danger" type="button">Cancel Join</button>`
        : `<button data-join="${r.id}" ${isFull ? "disabled" : ""} type="button">Join Ride</button>`;
      ...
      const postedByLine = `👤 Posted by ${poster}`;

      const contactLine = joined
        ? `📱 ${r.contact || ""}`
        : `📱 Contact unlocked once joined`;
      ...
      div.innerHTML = `
        ...
        <div class="rideMetaLine">🧭 ${escapeHtml(routeLine)}</div>
        <div class="rideMetaLine">${escapeHtml(postedByLine)}</div>
        <div class="rideSubMeta">💺 ${filled} / ${max} seats filled</div>
  ...
  // ====== JOIN / CANCEL ======
  async function joinRideClick(rideId) {
  ...
  async function doJoinRide(rideId) {
    ...
    // Prevent duplicate join (quick check)
  ...
  async function loadMyRidesClick() {
    ...
    let ok = false;
    try {
      ok = await withTimeout(requireAuthOrQueue({ type: "LOAD_MY", payload: {} }), 3000, "loadMy auth wait timeout");
    } catch (e) {
    ...
    if (!ok) {
      setText("my_msg", "");
      setText("my_err", "Still signing you in — please tap Sign in if prompted.");
      return;
    }
  ...
  async function loadMyRides() {
    ...
    const { data: posted, error: e1 } = await sb
      .from("rides")
      .select("*")
          ...
          canCancelJoin
            ? `
              <div class="row" style="margin-top:12px;">
                <button class="danger" data-cancel="${r.id}" type="button">Cancel Join</button>
              </div>
            `
            : `
              <div class="row" style="margin-top:12px;">
                <button class="danger" data-cancel-ride="${r.id}" type="button">Cancel Ride</button>
              </div>
  ...
  async function onAppResume() {
    ...
    try { await withTimeout(sb.auth.getSession(), 1200, "rehydrate getSession timeout"); } catch {}
    try { await withTimeout(sb.auth.refreshSession(), 2000, "refreshSession timeout"); } catch {}
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    $("find_date").addEventListener("change", () => {
      $("find_date_display").textContent = isoToMDY($("find_date").value);
    });
    $("post_date").addEventListener("change", () => {
      $("post_date_display").textContent = isoToMDY($("post_date").value);
    });
    ...
    // Resume listeners (no duplicates)
