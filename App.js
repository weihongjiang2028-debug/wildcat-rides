...
async function maybeResumePendingAction() {
  ...
  else if (pending.type === "CANCEL")       await doCancelJoin(pending.payload.rideId, null, true);
  else if (pending.type === "LOAD_MY")      { activateTab("my"); await loadMyRides(); }
  else if (pending.type === "CANCEL_RIDE")  await doCancelRideAsHost(pending.payload.rideId, null, true);
}
...
async function searchRides() {
  ...
  const enriched = list.map(r => {
    ...
    return {
      ...r,
      seats_filled:  filled,
      dist:          tMin === null ? null : Math.abs(mid - tMin),
